...
function handle_redirects(): void {
	 ...
	 * }
	 */
	$redirect_rules = apply_filters( __NAMESPACE__ . '_filters_redirect_rules', array() );
...
function path_matches_pattern( string $path, string $pattern ): bool {
	...
	$site_path = rtrim( ( $site_path ? $site_path : '' ), '/' );
...
function perform_redirect( array $rule, string $request_path ): void {
	...
	$valid_status_codes = array( 301, 302, 303, 307, 308 );
	 ...
	 */
	$redirect_url = apply_filters( __NAMESPACE__ . '_filters_redirect_url', $redirect_url, $rule, $request_path );
