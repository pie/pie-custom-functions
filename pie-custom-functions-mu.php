	...
	add_action(
		...
		function () {
			...
			printf(
				...
				esc_html__( '%1$s requires PHP %2$s or higher. You are running PHP %3$s.', 'pie-custom-functions' ),
...
require_once plugin_dir_path( __FILE__ ) . 'pie/redirections.php';
...
if ( is_multisite() ) {
	...
	/**
	 * Register meta box callbacks for user profile pages
	 *
	 * @since 1.0.0
	 * @return void
	 */
	...
	function custom_user_role_meta_box_callback( \WP_User $user ): void {
		...
		printf( '<option value="" %s>Select Role</option>', esc_attr( $selected ) );
		printf( '<option value="pie_admin" %s>Pie Admin</option>', esc_attr( $selected ) );
...
function staging_setup(): void {
	if ( is_multisite() ) {
		...
		if ( isset( $_SERVER['HTTP_HOST'] ) && false !== strpos( wp_unslash( $_SERVER['HTTP_HOST'] ), 'staging.tempurl.host' ) ) {
