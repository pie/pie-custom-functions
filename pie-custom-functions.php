...
function pie_custom_functions_init(): void {
	...
	if ( ! defined( 'WPMU_PLUGIN_DIR' ) ) {
		...
		wp_die(
			esc_html__( 'PIE Hosting Companion activation failed: MU plugins directory not available. Please contact your hosting provider.', 'pie-custom-functions' ),
			esc_html__( 'Plugin Activation Error', 'pie-custom-functions' ),
	...
	if ( ! copy( $local_mu_plugin_file, $destination_mu_plugin_file ) ) {
		...
		wp_die(
			esc_html__( 'PIE Hosting Companion activation failed: Could not copy MU plugin file. Please check file permissions.', 'pie-custom-functions' ),
			esc_html__( 'Plugin Activation Error', 'pie-custom-functions' ),
			array( 'back_link' => true )
		);
	}
	...
	if ( is_wp_error( $copy_result ) || ! $copy_result ) {
		...
		wp_die(
			esc_html__( 'PIE Hosting Companion activation failed: Could not copy pie directory. Please check file permissions.', 'pie-custom-functions' ) . ' ' . $error_message,
			esc_html__( 'Plugin Activation Error', 'pie-custom-functions' ),
			array( 'back_link' => true )
		);
	}
